...
def run_main_with_input(mock_password, mock_pwned_return=0, side_effect=None):
    ...
    """
    with patch("getpass.getpass", return_value=mock_password), patch(
        "utils.password_analyzer.PasswordAnalyzer.check_pwned",
        return_value=mock_pwned_return,
    ), patch("time.sleep", return_value=None), patch("builtins.print") as mock_print:

        if side_effect:
            with patch(
                "utils.password_analyzer.PasswordAnalyzer.__init__",
...
def test_pwned_api_error(monkeypatch):
    ...
    with patch(
        "utils.password_analyzer.PasswordAnalyzer.check_pwned", return_value=-1
